...
- **Two stages, three surfaces each.** The audit stage
  ([paniolo-scan](/.agents/skills/external/paniolo-scan/SKILL.md),
  ...
  ([paniolo-scan-remediate](/.agents/skills/external/paniolo-scan-remediate/SKILL.md),
...
- **Skills are vendored, not authored here.** Each `SKILL.md` under `.agents/skills/external/`
  is compiled from a wiki page in `paniolo-ai/sharp-shooter-wiki` (`wiki/skills/paniolo-<name>.md`)
  and shipped by `paniolo-ai/skills`. Change the wiki page, re-bundle, then
  `paniolo skills update` here — never edit the vendored copy; the change must stay
  self-contained, with no links to paths that exist only here.
...
- **Stable invocation.** The user-facing trigger is `/paniolo-scan`, and the install command is
  `npx skills add paniolo-ai/skills`. Do not rename these without updating every surface.
...
- **One version, one place.** `plugin.json` is the only hardcoded version — the plugin manifest
  requires it, so bump it on release. Vendored skills carry the `metadata.version` the source
  wiki page declares; `paniolo skills update` refreshes them via `skills-lock.json`.
