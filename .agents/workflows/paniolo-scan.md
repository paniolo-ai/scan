...
workflow doesn't either — remediation is a separate workflow: `/paniolo-scan-remediate`.
...
The audit changed nothing. Offer the two ways forward (using the `ask_question` tool or a text
query):
...
- The scanner is diagnostic-only, and this workflow is too: no file writes, no fixes.
